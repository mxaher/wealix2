 ...
 *
 * This is the interactive shell of the landing page.
 * It handles:
 * - Theme toggle
 * - Locale switching (EN/AR)
 * - Billing cycle toggle for pricing
 * - Clerk auth state (show/hide CTA buttons)
 * - Animations via framer-motion
 *
 * It does NOT affect SEO because the actual indexable HTML
 * is rendered server-side in src/app/page.tsx.
 */
...
const faqItems = {
  en: [
    ...
    ['How does the AI Advisor use my financial data?', 'When you use AI features, Wealix sends only the financial context needed for that request to the active AI processors. The Privacy Policy explains processing and retention in more detail.'],
  ...
  ar: [
    ...
    ['كيف يستخدم المستشار الذكي بياناتي المالية؟', 'عند استخدام ميزات الذكاء الاصطناعي، يرسل Wealix الحد الأدنى اللازم من السياق المالي المرتبط بطلبك إلى مزودات المعالجة النشطة. وتوضح سياسة الخصوصية تفاصيل المعالجة والاحتفاظ.'],
  ...
  return (
    ...
    >
      ...
      <main>
            ...
            >
              ...
              <h1 className="mt-6 max-w-3xl text-4xl font-semibold tracking-tight sm:text-5xl lg:text-6xl">
                {isArabic ? (
                  <>
                    أدر ثروتك برؤية{' '}
                    <span className="gradient-text">ذكية</span> وواضحة
                  </>
                ) : (
                  <>
                    Master your wealth with{' '}
                    <span className="gradient-text">intelligent</span> insights
                  </>
                )}
              </h1>
              ...
              <div className="mt-8 flex flex-wrap gap-3">
                {!clerkSignedIn && (
                  <Button
                    asChild
                    className="btn-primary rounded-xl px-5 py-6 text-sm"
                  >
                    <Link href="/sign-up">
                      {isArabic
                ...
                <Button
                  asChild
                  variant="outline"
                  className="rounded-xl border-border bg-card/70 px-5 py-6 text-sm"
                >
                  <a href="#features">
            ...
            >
              ...
              <div className="relative overflow-hidden rounded-[28px] border border-border bg-card/90 p-6 shadow-[0_30px_90px_-35px_rgba(0,106,255,0.35)]">
                <div className="flex items-center justify-between border-b border-border pb-5">
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground">
                      {isArabic
                        ? 'طبقة تشغيل مالية موحّدة'
                        : 'Unified financial operating layer'}
                    </p>
                  </div>
                ...
                <div className="mt-5 grid gap-4 lg:grid-cols-[1.05fr_0.95fr]">
                    ...
                    <div className="flex items-center justify-between">
                      <div>
                        <p className="text-sm text-muted-foreground">
                          {isArabic ? 'صحة المحفظة' : 'Portfolio Health'}
                        </p>
                        <p className="mt-1 text-2xl font-semibold financial-number">
                          612,450 SAR
                        </p>
                      </div>
                  ...
                  <div className="card-hover rounded-[20px] border border-border bg-background p-5">
                    <p className="text-sm text-muted-foreground">
                      {isArabic ? 'تركيز الذكاء الاصطناعي' : 'AI Focus'}
                    </p>
          ...
          <div className="mx-auto max-w-7xl rounded-[28px] border border-border bg-card/70 p-6 sm:p-8">
            <div className="grid gap-5 md:grid-cols-2">
              {[
                ...
                {
                  titleEn: 'Technical safeguards',
                  titleAr: 'الحماية التقنية',
                  bodyEn:
                ...
                {
                  titleEn: 'AI data processing',
                  titleAr: 'معالجة بيانات الذكاء الاصطناعي',
                  bodyEn:
                    'AI advisor prompts and portfolio analysis are sent to NVIDIA APIs, and OCR requests may be processed by NVIDIA or Datalab. Only the minimum financial context needed for the request is sent.',
                  bodyAr:
                    'تُرسل أسئلة المستشار الذكي وتحليل المحفظة إلى واجهات NVIDIA، كما قد تُرسل طلبات OCR إلى NVIDIA أو Datalab بحسب المسار التشغيلي.',
                },
                {
                  titleEn: 'Regulatory disclosure',
                  ...
                  bodyAr:
                    'Wealix ليست بنكاً أو شركة وساطة أو مستشاراً استثمارياً مرخّصاً من البنك المركزي السعودي، ولا تحتفظ بأصول المستخدمين ولا تنفذ الصفقات.',
                },
              ].map((card) => (
                <div
                  key={card.titleEn}
                  className="rounded-[20px] border border-border bg-background/80 p-5"
                >
        ...
        <section
          id="pricing"
          className="border-y border-border bg-secondary/35 px-4 py-20 sm:px-6 lg:px-8"
        >
          <div className="mx-auto max-w-6xl">
            <div className="mx-auto max-w-2xl text-center">
              ...
              <h2 className="mt-5 text-3xl font-semibold tracking-tight sm:text-4xl">
                {isArabic
                  ? 'اختر خطتك ثم ابدأ تجربة 14 يوماً'
                  : 'Choose your plan, then start a 14-day trial'}
              </h2>
              ...
              {pricingByCycle[billingCycle].map((plan) => (
                <div
                  ...
                  className={`card-hover flex flex-col rounded-[24px] bg-card p-8 ${
                    plan.id === 'pro'
                      ? 'border border-primary/20 ring-1 ring-primary/10'
            ...
            <div className="flex flex-col gap-6 md:flex-row md:items-end md:justify-between">
              ...
              <div className="flex flex-wrap gap-3">
                ...
                {clerkSignedIn && (
                  <Button asChild className="btn-primary rounded-xl">
                    <Link href="/app">
                      {isArabic ? 'اذهب إلى التطبيق' : 'Go to app'}
                    </Link>
                  </Button>
                )}
              </div>
